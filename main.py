...
try:
    ...
    result_region = []
    result_score = []
    result = [result_ip, result_asn, result_region, result_score]
                ...
                while i < 10:
                    try:
                        ...
                        while fetchList < len(getList):
                            ...
                            if fetchList == 1:
                                result_asn.append([element.text])
                            if fetchList == 3:
                                result_region.append([element.text])
                            if fetchList == 4:
                                result_score.append([element.text])
                            fetchList += 1
                        break
                    except Exception as err:
                        print(err)
    ...
    with open('result.txt','w', encoding="utf-8") as file:
        lps = 0
        strings = []
        while lps <= len(result):
            strings.append(f'{result[lps]} == { result[lps+1]} == {result[lps+2]} == {result[lps+3]}')
        for stg in strings:
            file.write(stg)
